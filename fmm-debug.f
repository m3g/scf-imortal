...
c Computing F' = Ut F U
        ...
        call pmatrix(kdim, kdim, kdim, fx, u, temp)
        call pmatrix(kdim, kdim, kdim, ut, temp, fl)         
...
c Diagonalize F'
        ...
        call pmatrix(kdim,kdim,kdim,fl,temp,temp)
        call pmatrix(kdim,kdim,kdim,xy,temp,temp)

        do i = 1, kdim
          do j = 1, kdim
            test = abs(temp(i,j) - debug(i,j))
            if(test.gt.1.d-8) write(*,*) i, j, test
          end do
        end do
...
c Testing Ut S U = I
      ...
      call pmatrix(kdim,kdim,kdim,s,u,temp)
      call pmatrix(kdim,kdim,kdim,ut,temp,temp)

      do i = 1, kdim
        do j = 1, kdim
          if(i.eq.j) then
            test = abs(temp(i,j) - 1.d0)
          else
            test = abs(temp(i,j))
          end if
          if(test.gt.1.d-8) write(*,*) ' ERROR: ut s u = ? ', i, j,test
        end do
      end do

c Computing the matrix Y from the eigenvectors of F'

        call pmatrix(kdim, kdim, kdim, u, xy, temp) 
        ...
        do i = 1, ndm
          do j = 1, ndm
            ...
            if(test.gt.1.d-8) then
              write(*,*) ' FATAL ERROR: ', i, j, ' test = ', test
            end if
          end do
        end do
...
c Testing the feasibility of the current point
     ...
     *                         'Current Point is not feasible '
                    testfile = 'CURRENTNOTFEASIBLE_'//current(9:80)
                    open(10,file=testfile)
                    do ii = 1, kdim
...
c Testing the feasibilty of the trial point
     ...
     *                         'Trial Point is not feasible '
                    testfile = 'TRIALNOTFEASIBLE_'//current(9:80)
                    open(10,file=testfile)
                    do ii = 1, kdim
     ...
     *                     ' area feasible and Pred < 0: '
                ...
                testfile = 'DATA_'//current(9:80)
                open(10,file=testfile)
                open(23,file='MARMOT_'//testfile)
                write(10,*) ' k = ', kdim, ' n = ', ndm
                write(10,*) ' PRed = ', pred

                test = 0.d0
                write(23,*) ' S = '
                do i = 1, kdim
                  write(23,698) (s(i,j), j = 1, kdim)
                  do j = 1, kdim
                    test = test + s(i,j)
                  end do
                end do
                write(*,*) ' suma de s(i,j) = ', test

                test = 0.d0
                write(23,*) ' Current point = '
                do i = 1, kdim
                  write(23,698) (xx(i,j), j = 1, ndm)
                  do j = 1, kdim
                    test = test + xx(i,j)
                  end do
                end do
                write(*,*) ' soma de xx(i,j) = ', test

                test = 0.d0
                write(10,*) ' Trial point = '
                do i = 1, kdim
                  write(10,698) (xy(i,j), j = 1, ndm)
                  do j = 1, kdim
                    test = test + xy(i,j)
                  end do
                end do
                write(*,*) ' soma de xy(i,j) = ', test

                call computefe(xx,fx,ex)
                write(*,*) ' Energia en el current point = ', ex
                test = 0.d0
                write(23,*) ' F en el current point = '
                do i = 1, kdim
                  write(23,698) (fx(i,j), j = 1, kdim)
                  do j = 1, kdim
                    test = test + fx(i,j)
                  end do
                end do
                write(*,*) ' soma de fx(i,j) = ', test

                call pmatrix(kdim, kdim, kdim, fx, u, temp)
                call pmatrix(kdim, kdim, kdim, ut, temp, fl)         
                call diag(fl,temp,kdim) 
                do i = 1, kdim
                  write(*,*) fl(i,i)
                end do

                test = 0.d0
                write(10,*) ' F en el trial point = '
                do i = 1, kdim
                  write(10,698) (fy(i,j), j = 1, kdim)
                  do j = 1, kdim    
                    test = test + fy(i,j)
                  end do
                end do
                write(*,*) ' soma de fy(i,j) = ', test

                close(23)
                close(10)
698             format(100(tr2,e17.10))
              end if
              stop
