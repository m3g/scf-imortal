...
C*********************************************************
      ...
      real*8 ABCDtp(maxd,maxd,maxd,maxd),rnorm(maxbase)
      real*8 Htp(maxd,maxd), Stp(maxd,maxd)
    
      LOGICAL WTODO
c      logical uncont, efield

c ?   COMMON /HF/ P(maxbase,maxbase),RNORM(maxbase),EHF,NSCF

      double precision intvalues(maxd*maxd*maxd*maxd)
      common / intvalues / intvalues
...
c      open(20,file="intvalues.dat",access="direct",
c     *        form="unformatted",recl=17) 
...
c            alphac(i,j,k) = 0.d0
              do l = 1, ncont(i,j,k)
                iang = iang + 1
c                alphac(i,j,k) = alphac(i,j,k) + 
c     *                          alpha(i,j,iang)*cont(i,j,k,l)
              end do
            end do
          end do
        end do
...
c Adicionado para escrever as integrais em arquivo:
     ...
     !I4,J4,K4,LX1,LY1,LZ1,LX2,LY2,LZ2,LX3,LY3,LZ3,LX4,LY4,LZ4)
      ...
      intvalues(index) = ABCDtemp

c      write(20,rec=index) ABCDtemp
